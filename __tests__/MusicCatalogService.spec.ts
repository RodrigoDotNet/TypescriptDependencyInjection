...
describe("MusicCatalogService tests", () => {
  ...
  it("Should return Tracks value", () => {
    //Arrange
    let mockMusicRepository:IMusicRepository =mock<IMusicRepository>();
    when(mockMusicRepository.get()).thenReturn(mockRepo);

    sut = new MusicCatalogService(instance(mockMusicRepository));

    //Act
    var result = sut.get();

    //Assert
    verify(mockMusicRepository.get()).once();
  ...
  it("Should add Track", () => {
    //Arrange
    const Mock = jest.fn<IMusicRepository>(() => ({
      add: jest.fn().mockImplementation((track: Track) => {
  ...
  it("Should edit Track", () => {
    //Arrange
    const Mock = jest.fn<IMusicRepository>(() => ({
      edit: jest.fn().mockImplementation((id: number, track: Track) => {
        return track;
      }),
    }));
    const mock = new Mock();
